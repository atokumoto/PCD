...
int conta_vivos(int**grid){//conta o numero de celulas vivas no tabuleiro
    ...
    #pragma omp parallel private(j) reduction (+:total)
    #pragma omp for
	    for(i=0;i<TAM_TAB;i++){
		    for(j=0;j<TAM_TAB;j++){
...
void geracoes(int num_ger){
	...
	for(k=0;k<num_ger;k++){//passa as gerações
        ...
        #pragma omp parallel private(j,v_vivos) shared(newgrid)
        #pragma omp for	
		    for(i=0;i<TAM_TAB-1;i++){
			    ...
			    for(j=0;j<TAM_TAB-1;j++){
				    ...
				    if(grid[i][j]==1){//celula esta viva
					    ...
					    if(v_vivos == 3 ||v_vivos == 6) newgrid[i][j]= 1;
					    else newgrid[i][j]= 0;
			    	}				
			    }
		    }
